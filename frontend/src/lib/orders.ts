...
// kitchen" round) — priority is: anything still being cooked > food ready
// and waiting > everything cooked but nobody's paid yet > nothing open.
export function deriveTableStatus(ordersForTable: OrderRow[] | undefined): TableStatus {
  ...
  if (ordersForTable.some((o) => o.kitchen_status === 'new' || o.kitchen_status === 'cooking')) return 'occupied';
...
export function combineOrderLines(ordersForTable: OrderRow[]): CombinedLine[] {
  return ordersForTable.flatMap((o) =>
    o.items.map((i) => ({ quantity: i.quantity, name: i.name, unit_price: i.unit_price, note: i.note ?? undefined }))
  );
}
